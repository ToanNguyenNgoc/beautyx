...
import { EmptyRes, ProductableItemGroup, XButton } from "components/Layout";
...
function TabServiceProductableGroup({ keyword }: { keyword: string }) {
    ...
    <>
      ...
      <div className={style.result_body}>
        ...
        >
          <ul>
            {
              serviceData.productableGroup.map(item => (
                <li className={style.productableGroupItem} key={item.organization.id}>
                  {/* <ProductableItem productable={item} changeStyle={IS_MB} /> */}
                  <ProductableItemGroup item={item} />
                </li>
              ))
            }
          </ul>
...
export default TabServiceProductableGroup